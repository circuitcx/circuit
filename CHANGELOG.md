...
### Changed
- Clarified wording: dropped the misleading "LLM-free" phrasing. Circuit is
  AI-powered (your Claude does the classification); the server is the
  deterministic data layer that makes no model calls itself.
...
- `circuitcx` MCP server — deterministic Gmail engine (makes no model calls
  itself; classification is done by your AI client) exposing
