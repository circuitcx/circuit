...
[![npm](https://img.shields.io/npm/v/circuit-mcp.svg)](https://www.npmjs.com/package/circuit-mcp)
...
npx -y circuit-mcp auth      # one-time, read-only Gmail consent
```
...
| | Skill (`/circuit`) | + MCP server (`circuit-mcp`) |
|---|---|---|
| Install | plugin / copy to `~/.claude/skills` | `npx -y circuit-mcp` |
...
```
   ...
   Claude  (your AI client)
       │   calls a Gmail tool:
       ├──────────────►  circuit-mcp server  ──►  Gmail API (read-only)
...
├── mcp-server/               circuit-mcp — deterministic Gmail engine (npm)
