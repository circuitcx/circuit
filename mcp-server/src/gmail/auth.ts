...
//
// One-time setup: the user runs `circuit-mcp auth`, which opens a browser,
...
export async function getAccessToken(): Promise<string> {
  ...
  if (!token || !token.refresh_token) {
    throw new Error(
      "Not authenticated. Run `npx circuit-mcp auth` once to connect your Gmail account.",
    );
  }
...
/** Interactive one-time OAuth flow. Run via `circuit-mcp auth`. */
  ...
  if (!data.refresh_token) {
    throw new Error(
      "No refresh token returned. Revoke prior access at " +
        "https://myaccount.google.com/permissions and run `circuit-mcp auth` again.",
    );
  }
  await saveToken({
    access_token: data.access_token,
